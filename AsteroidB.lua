...
function AsteroidB:init(x,size)
    
    self.position = vec2(x,800)
    self.size = size
    self.body = physics.body(CIRCLE, size)
    
end

function AsteroidB:draw()
    sprite("Space Art:Asteroid Small",self.position.x, self.position.y, self.size, self.size)
    self:move()
    pushStyle()
end
...
function AsteroidB:move()
    ...
    if self.position.y <= 0 then
        ...
        AsteroidB:init(math.random(0,WIDTH), math.random(20,300))
    end
