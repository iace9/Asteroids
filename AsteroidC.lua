...
function AsteroidC:init(x,size)
    
    self.position = vec2(x,1000)
    self.size = size
    self.body = physics.body(CIRCLE, size)
    
end

function AsteroidC:draw()
...
function AsteroidC:move()
    ...
    if self.position.y <= 0 then
        ...
        AsteroidC:init(math.random(0,WIDTH), math.random(20,300))
    end
